  ...
  class sml_env_config extends sml_env {
        ...
        if(true) {
            ...
            if(file_exists($app_config_file)) {
                ...
                if(isset($app_config["subs"]["bdd"][0]["subs"]["table_prefix_code"])) {
                    $this->add_table_prefix(array($app_config["subs"]["bdd"][0]["subs"]["table_prefix_code"][0]["data"]=>$bdd["table_prefix"]));
                }
            }
            if(($plugins=$this->plugins("ASC"))!==false) {
                ...
                $this->erreur("impossible de lire les fichiers de configuration pour les plugins",true);
        }
        else 
            $this->erreur("impossible de trouver le fichier de configuration pour l'installation",true);
    }
    ...
    function set_PATHES($PATHES) {
        foreach($PATHES as $path_name=>$path_value) {
            if($path_value&&substr($path_value,-1)!="/") 
                $PATHES[$path_name].="/";
        }
        $this->PATHES=$PATHES;
    }
    ...
    function add_table_prefix($table_prefix) {
        if(is_array($table_prefix)) {
            foreach($table_prefix as $prefix_code=>$prefix) 
                $this->bdd["table_prefix"][$prefix_code]=$prefix;
            return true;
        }
        return false;
    }
