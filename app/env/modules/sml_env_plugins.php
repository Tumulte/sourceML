<?php
  ...
  class sml_env_plugins extends sml_env {
    ...
    function plugins($PRIORITE="ASC") {
        ...
        if($PRIORITE=="ASC") 
            return $this->plugins_asc;
        if($PRIORITE=="DESC") 
            return $this->plugins_desc;
        return false;
    }
    # ---------------------------------------------------------------------------------
    #                                                                              init
    ...
    function init_plugins($PRIORITE="ASC") {
        if(isset($this->plugins_asc)||isset($this->plugins_desc)) {
            if($PRIORITE=="ASC") {
                if(!isset($this->plugins_asc)) 
                    $this->plugins_asc=$this->ordonne_plugins($this->plugins_desc,$PRIORITE);
            }
            elseif($PRIORITE=="DESC") {
                if(!isset($this->plugins_desc)) 
                    $this->plugins_desc=$this->ordonne_plugins($this->plugins_asc,$PRIORITE);
            }
            return;
        }
        $plugins=array();
                    ...
                    if(substr($plugin_name,0,1)!=="."&&is_dir($this->path("plugins").$plugin_name)) {
                        if(!isset($plugins[$plugin_name])) {
                            if(($plugin=$this->plugin_data($plugin_name))!==false) {
                                ...
                                if($MAJ) 
                                    $OK=$this->set_plugin_data($plugin_name,$plugin);
                                if($OK) {
                            ...
                            while($OK&&($plugin_name=readdir($dh))!==false) {
                                if(substr($plugin_name,0,1)!="."&&!is_dir($this->plugin_data_file($plugin_name))) {
                                    if(!$plugins[$plugin_name]) 
                                        $this->del_plugin_data($plugin_name);
                                }
                                if(!$OK) 
                                    $plugins=false;
                            }
        ...
        if($plugins!==false) {
            if($PRIORITE=="ASC") 
                $this->plugins_asc=$this->ordonne_plugins($plugins,$PRIORITE);
            elseif($PRIORITE=="DESC") 
                $this->plugins_desc=$this->ordonne_plugins($plugins,$PRIORITE);
        }
        else {
    ...
    function ordonne_plugins($plugins,$PRIORITE="ASC") {
        ...
        while($maximum>0) {
            ...
            for($i=0;$i<$maximum-1;$i++) {
                if(($PRIORITE=="ASC"&&$values[$i]["priorite"]>$values[$i+1]["priorite"])||($PRIORITE=="DESC"&&$values[$i]["priorite"]<$values[$i+1]["priorite"])) {
                    $tmp=$values[$i];
                    $values[$i]=$values[$i+1];
                    $values[$i+1]=$tmp;
