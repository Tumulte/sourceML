<?php
  ...
  class sml_data_groupes extends sml_data {
    ...
    function groupes($id_user=null,$start=null,$alpha=null) {
        ...
        if($groupes["total"]>0) {
            ...
            while($v_rst=$sgbd->fetch_assoc($rst)) {
                ...
                $groupes["list"][$v_rst["id"]]["image_uri"]=($v_rst["image"]?$env->path("content")."uploads/".$v_rst["image"]:"");
            }
            $sgbd->free_result($rst);
        }
        return $groupes;
    ...
    function groupe($id) {
        ...
        if($v_rst=$sgbd->fetch_assoc($rst)) {
            $groupe=$v_rst;
            $groupe["image_uri"]=($groupe["image"]?$env->path("content")."uploads/".$groupe["image"]:"");
        }
        $sgbd->free_result($rst);
        return $groupe;
    }
    function groupe_exists($nom,$other_than_id=null) {
        ...
        if(isset($other_than_id)) 
            $sql.=" AND id!=".$this->eq($other_than_id);
        $rst=$sgbd->query($sql);
        if(!isset($rst)) 
            return false;
        if($v_rst=$sgbd->fetch_assoc($rst)) 
    ...
    function set_groupe($id,$nom,$image,$description,$email,$contact_form,$captcha) {
        if(($groupe=$this->groupe($id))!==false) {
            $sgbd=$this->sgbd();
            $sql="UPDATE #--groupes SET"."  nom=".$this->eq($nom).", image=".$this->eq($image).", description=".$this->eq($description).", email=".$this->eq($email).", contact_form=".$this->eq($contact_form).", captcha=".$this->eq($captcha)." WHERE id=".$id;
            if($sgbd->query($sql)) {
                if($nom!=$groupe["nom"]) {
                    $groupe["nom"]=$nom;
                    if(!$this->maj_source_xml_groupe($groupe)) 
                        return false;
                }
                return true;
            }
