<?php
  ...
  class sml_users_groupes extends sml_mod {
    ...
    function validate(&$env) {
        ...
        if($this->user=$env->user()) {
            if($env->etat("action")=="edit"||$env->etat("action")=="del") {
                if(($this->groupe=$data->groupe($_GET[$env->param("id")]))!==false&&$this->groupe) {
                    if($this->groupe["id_user"]==$this->user["id"]) {
                        return true;
                    }
                    else 
                        return "Vous n'est pas autoris&eacute; &agrave; modifier ce groupe";
                }
                else 
    ...
    function index(&$env) {
        $data=$env->data();
        if($env->set_out("groupes",$data->groupes($this->user["id"],isset($_GET[$env->param("start")])?$_GET[$env->param("start")]:0))!==false) {
        }
        else 
                        ...
                        if(($exists=$data->groupe_exists($_POST["nom"]))!==false) {
                            if(!$_POST["contact_form"]||trim($_POST["email"])) {
                                if($exists==0) {
                                    ...
                                    $env->message("Un groupe avec ce nom existe d&eacute;j&agrave;");
                            }
                            else 
                                $env->message("merci de pr&eacute;ciser un email pour le formulaire de contact");
                        }
                        else 
                            $env->erreur("Impossible de savoir si le groupe existe d&eacute;j&agrave;");
                    }
                    else 
                        $env->message("merci de pr&eacute;ciser un nom pour le groupe");
                }
                else 
        ...
        if($data->check_user_uploads_dir()) {
            ...
            if($env->set_out("groupe",$this->groupe)) {
                ...
                if($_POST) {
                    ...
                    if($_POST["del_image"]) {
                        if($image) {
                            if(@unlink($env->path("content")."uploads/".$image)) 
                                $image="";
                            else {
                                $image=false;
                    ...
                    if($image!==false) {
                        if(trim($_POST["nom"])) {
                            if(($exists=$data->groupe_exists($_POST["nom"],$this->groupe["id"]))!==false) {
                                if(!$exists) {
                                    if(!$_POST["contact_form"]||trim($_POST["email"])) {
                                        if($data->set_groupe($id,$_POST["nom"],$image,$_POST["description"],$_POST["email"],$_POST["contact_form"]?1:0,$_POST["captcha"]?1:0)) 
                                            $env->redirect($env->url("users/groupes/edit",array("id"=>$_GET[$env->param("id")])),"le groupe a &eacute;t&eacute; modifi&eacute;");
                                        else 
                                            $env->erreur("Impossible de modifier le groupe");
                                    }
                                    else 
                                        $env->message("merci de pr&eacute;ciser un email pour le formulaire de contact");
                                }
    ...
    function del(&$env) {
        ...
        if(($res=$data->del_groupe($this->groupe["id"]))!==false) {
            if($res===1) 
                $env->redirect($env->url("users/groupes"),"Ce groupe a des sources sur ce site, il n'a pas &eacute;t&eacute; supprim&eacute;.",5);
            else 
                $env->redirect($env->url("users/groupes"),"le groupe a &eacute;t&eacute; supprim&eacute;");
        }
        else 
            $env->erreur("Impossible de supprimer le groupe");
    }
}
?>
